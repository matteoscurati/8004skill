...
2. Use helpers from `scripts/lib/shared.ts` — at minimum `parseArgs()` for argument parsing and `handleError()` as the catch handler. Other utilities: `requireArg`, `parseChainId`, `parseIntStrict`, `validateAgentId`, `validateAddress`, `validateIpfsProvider`, `buildSdkConfig`, `exitWithError`.
