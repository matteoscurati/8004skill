...
| Agent URI | Pointer to an agent's registration file. Can be IPFS (`ipfs://...`) or HTTP(S). |
...
| Feedback File | Off-chain IPFS document enriching feedback with text, context, and proof of payment. |
...
| Registration File | JSON document (on IPFS/HTTP) containing agent profile: name, description, endpoints, and capabilities. |
...
| Validation Registry | Third registry for validator attestations. Limited SDK support as of v1.5.3. |
